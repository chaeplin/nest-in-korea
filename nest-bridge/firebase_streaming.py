...
# def firebaseURL(URL): changed by chaeplin
...
class RemoteThread(threading.Thread):
    ...
    def run(self):
        try:
            self.sse = ClosableSSEClient(self.URL, timeout=60)            
            for msg in self.sse:
...
#                URL = URL[:-1]
#            URL = 'https://' + \
#                URL.split('/')[0] + '.nest.com/' + URL.split('/', 1)[1] 
#        else:
#            URL = 'https://' + URL + '.nest.com/'
#        return URL
#
#    if 'http://' in URL:
